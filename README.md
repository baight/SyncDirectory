...
- **两种目录结构**
  ...
  - **平铺模式**：忽略源目录层级，所有文件直接放到目标根目录（`a\b.txt` 将映射为 `a_b.txt`，自动处理重名冲突）
...
- **配置管理**：源/目标目录与各种模式可保存为配置，支持重命名、加载、删除；配置文件存放在 exe 同级目录，随程序一起移动
...
| | 原始结构 | 平铺 |
| --- | --- | --- |
| **增量同步** | 按源目录层级复制目标缺少的文件，不删除不覆盖 | 所有文件平铺复制到目标根目录，不删除不覆盖（目标已有子目录被忽略） |
| **完全同步** | 复制缺失文件并删除目标多余内容，两目录完全一致 | 所有文件平铺到目标根目录，并删除目标中的子目录与多余文件 |

> 同名但大小不同的文件：增量同步下跳过并记录原因；完全同步下覆盖复制。同名但类型不同（一边是文件、一边是目录）的情况会按类型冲突处理并给出提示。
...
配置文件 `syncdirectory.config.json` 自动生成在 exe 同级目录；开发模式下生成在项目根目录。
...
- **Two layout modes**
  ...
  - **Flat**: ignore the hierarchy and put all files directly into the target root (`a\b.txt` is mapped to `a_b.txt`, with automatic conflict resolution)
...
- **Config management**: save directory pairs and modes as named configs; rename, load, and delete them. The config file lives next to the exe, so it travels with the app
...
| | Original | Flat |
| --- | --- | --- |
| **Incremental** | Copy missing files following the source hierarchy; no delete, no overwrite | Copy all files flat into the target root; no delete, no overwrite (existing subdirectories are ignored) |
| **Full** | Copy missing files and delete extras — both directories end up identical | Put all files flat into the target root and delete subdirectories/extras |

> Files with the same name but different sizes are skipped (with reasons logged) in incremental mode, and overwritten in full mode. Name collisions between a file and a directory are treated as type conflicts with warnings.
...
The config file `syncdirectory.config.json` is created next to the exe (in the project root during development).
